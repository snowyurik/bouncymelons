...
class BouncyMelons {
	 ...
	 * @return array of associated array, each sub array represent a row
	 */
	public function select($table, $where, $limit = null, $orderby = null, $groupBy = null) {
		$wr = '';
		if(!empty($where)) {
			$wr = "WHERE ".$where;
		}
...
//		var_dump("SELECT * FROM `".$table."` ".$wr." ".$gb." ".$ob." ".$lim);
		return $this->query("SELECT * FROM `".$table."` ".$wr." ".$gb." ".$ob." ".$lim);
	}
...
interface IField {
	...
	*/
	 ...
	 */
	public function beforeSet($value, ISingle $single);
	
	/**
	 * change field value, for proper display in nice way, unserialize, etc
	 */
	public function beforeRead($value, ISingle $single);
	
	/**
	 * change field value, for editing it later
	 */
	public function beforeGet($value, ISingle $single);
...
class ERequireFieldOptionWrong extends Exception {}
...
class DateField extends Field {
	...
	public function beforeGet($value, ISingle $single) {
		$val = parent::beforeGet($value, $single);
...
class PhoneField extends Field {
	const DEFAULT_SQL_TYPE = "varchar(10)";
...
class SetField extends Field {
	...
	public function __construct($name, $title = null, $options = array(), $sqlType = null) {
		...
		if(!is_array($options['set'])) {
			throw new ERequireFieldOptionWrong("\bm\SetField field option 'set' should be one-dimentional assosiative array, check docs for more info" );
		}
...
class BoolField extends Field {
	...
	public function beforeSet($value, ISingle $single) {
		...
		return parent::beforeSet($value, $single);
	}
}
...
//		$data = [];
//		foreach($this->getFields() as $field) {
//			$data[$field->getName()] = $data
//		}
		$id = $bm->insertOrUpdate($this->getTableName(),$this->getData());
		$this->setId($id);
		return $id;
	}
	...
	public function get($fieldName) {
		...
		return $field->beforeGet($this->getRaw($fieldName), $this);
	}
	...
	public function set($fieldName, $value) {
		try{
			$field = $this->getFieldByName($fieldName);
			$this->data[$fieldName] = $field->beforeSet($value, $this);
			return;
...
abstract class Filter implements IFilter {
	...
	public function setGroupBy($groupBy) {
		...
		/**
	 * @return int 50, but can be overriden in child classes
	 */
